...
public class UnreadChainTest
{
    ...
    public void Test1()
    {
        ...
        g.IdChain.Select(x => x.Id).SequenceEqual(new int[] { -2, 1, 2, 3, }).IsTrue();
        g.Id2Chain.Select(x => x.Id2).SequenceEqual(new int[] { -2, 1, 2, 3, }).IsTrue();
        g.Id3Chain.Select(x => x.Id3).SequenceEqual(new int[] { }).IsTrue();
        ...
        g.Id3Chain.Select(x => x.Id3).SequenceEqual(new int[] { -2, 1, 2, 3, }).IsTrue();

        // g -> g2
        var g2 = TinyhandSerializer.Deserialize<UnreadChainTestClass.GoshujinClass>(TinyhandSerializer.Serialize(g));
        g2!.IdChain.Select(x => x.Id).SequenceEqual(new int[] { -2, 1, 2, 3, }).IsTrue();
        g2!.Id2Chain.Select(x => x.Id2).SequenceEqual(new int[] { -2, 1, 2, 3, }).IsTrue();
        g2!.Id3Chain.Select(x => x.Id3).SequenceEqual(new int[] { -2, 1, 2, 3, }).IsTrue();
        ...
        g3!.IdChain.Select(x => x.Id).SequenceEqual(new int[] { -2, 1, 2, 3, }).IsTrue();
        ...
        g2!.IdChain.Select(x => x.Id).SequenceEqual(new int[] { -2, 1, 2, 3, }).IsTrue();
        g2!.Id2Chain.Select(x => x.Id2).SequenceEqual(new int[] { -2, 1, 2, 3, }).IsTrue();
        g2!.Id3Chain.Select(x => x.Id3).SequenceEqual(new int[] { }).IsTrue();
    }
}
