...
public partial record JournalTestClass : IEquatableObject
{
    ...
    [Key("id", PropertyName = "Id")]
    private int id;

    [Link(Type = ChainType.Ordered)]
    [Key("identifier", PropertyName = "Identifier")]
    private JournalIdentifier identifier = default!;

    [Key("name", PropertyName = "Name")]
...
public class JournalTest
{
    ...
    public void TestGoshujin()
    {
        ...
        JournalHelper.ReplayJournal(g3, journal).IsTrue();
    ...
    public void TestGoshujin2()
    {
        ...
        using (var w = g2.TryLock(new JournalIdentifier(20), AcquisitionMode.GetOrCreate))
        {
            w!.Name = "20";
            w!.Id = new(222);
        ...
        using (var w = g2.TryLock(new JournalIdentifier(5), AcquisitionMode.GetOrCreate))
        {
            if (w is not null)
            {
                w.Name = "Fi";
        ...
        for (var i = 0; i < 100; i++)
        {
            using (var w = g2.TryLock(new JournalIdentifier(i), AcquisitionMode.GetOrCreate))
            {
                if (w is not null)
                {
                    w.Commit();
        ...
        using (var w = g2.TryLock(new JournalIdentifier(60)))
        {
            if (w is not null)
            {
                ...
                using (var w2 = w.Children.TryLock(new JournalIdentifier(10), AcquisitionMode.GetOrCreate))
                {
                    if (w2 is not null)
                    {
                        w2.Name = "Y";
        ...
        ((IRepeatableReadSemaphore)g2).LockAndTryRelease(out var state).IsTrue();
